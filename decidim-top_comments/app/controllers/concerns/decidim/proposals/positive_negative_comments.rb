    ...
    module PositiveNegativeComments
      ...
      included do
        ...
        def most_voted_in_favor_comments
          @most_voted_in_favor_comments = in_favor_comments
                                          .joins(:up_votes)
                                          .select("decidim_comments_comments.*, SUM(decidim_comments_comment_votes.weight) AS vote_sum")
                                          .group("decidim_comments_comments.*, decidim_comments_comments.id")
                                          .order(Arel.sql("vote_sum DESC"))
        end
        ...
        def most_voted_against_comments
          @most_voted_negative_comments = against_comments
                                          .joins(:up_votes)
                                          .select("decidim_comments_comments.*, SUM(decidim_comments_comment_votes.weight) AS vote_sum")
                                          .group("decidim_comments_comments.*, decidim_comments_comments.id")
                                          .order(Arel.sql("vote_sum DESC"))
